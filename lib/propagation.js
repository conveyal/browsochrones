...
import type {Origin, Query, StopTreeCache} from './types'

type NextParameters = {
  travelTimesForDest: Uint8Array,
  walkTimesForDest: Uint8Array,
  inVehicleTravelTimesForDest: Uint8Array,
  waitTimesForDest: Uint8Array,
  x: number,
  y: number
}

/**
 * Perform propagation from an origin to destination cells, calling a callback
 * to summarize the data. This seems like it's overly complicated, but I don't
 * think there's an easier way to have this code used both to create surfaces,
 * isochrones and spectrogram data, which are different summaries of the data.
 *
 * The callback will be called once for each destination pixel with:
...
export default function propagate ({
  ...
  for (let y = 0, pixelIdx = 0, stcOffset = 0; y < query.height; y++) {
    for (let x = 0; x < query.width; x++, pixelIdx++) {
      ...
      // when the origin is within walking distance and walking is the fastest
      // way to reach the destination, _everything_ is walk time
      walkTimesForDest.fill(nonTransitTime)
      ...
      for (let stopIdx = 0; stopIdx < nStops; stopIdx++) {
        ...
        for (let minute = 0; minute < nMinutes; minute++) {
          ...
          if (travelTimeToStop !== -1) {
            ...
            // no need to check that travelTimeToPixel < 255 as
            // travelTimesForDest[minute] is preinitialized to the nontransit time or 255
