...
export default class Browsochrones {
  ...
  generateSurface (cutoff = 60) {
    return new Promise((resolve, reject) => {
      if (!this.isLoaded()) {
        reject(new Error('Surface cannot be generated if Browsochrones is not fully loaded.'))
      }

      /* const worker = workify(getSurface)

      worker.addEventListener('message', (event) => {
        this.surface = event.data
      ...
      worker.postMessage(input, [this.origin.data, this.origin.index, this.query, this.stopTrees, this.which]) */
  ...
  setOrigin (data, coordinates) {
    this.origin = createOrigin(new Int32Array(data), coordinates)
  }
  ...
  setStopTrees (data) {
    ...
    this.stopTrees = createStopTreeCache(new Int32Array(data), this.query.width * this.query.height)
  }
