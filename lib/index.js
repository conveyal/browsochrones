...
export default class Browsochrones extends WebWorkerPromiseInterface {
  ...
  getAccessibilityForGrid (grid, cutoff = 60) {
    if (!this.isLoaded()) {
  ...
  generateSurface (grid, which = 'MEDIAN') {
    if (!this.isLoaded()) {
  ...
  latLonToOriginPoint ({lat, lon, lng}) {
    ...
    if (!this.query) throw new Error('Cannot convert lat/lon without query.')

    let {north, west, zoom} = this.query

    let {x, y} = LeafletMap.prototype.project([lat, lon], zoom)
    ...
    if (!this.pointInQueryBounds(ret)) throw new Error('Point out of query bounds!')
