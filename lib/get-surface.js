 ...
 */
export default function getSurface ({grids, origin, query, stopTreeCache, which}) {
  ...
  let access = new Map()
  for (let [name] of grids) {
    access.set(name, new Int32Array(nMinutes * 120))
  }
  ...
  for (let y = 0, pixelIdx = 0, stcOffset = 0; y < query.height; y++) {
    for (let x = 0; x < query.width; x++, pixelIdx++) {
      ...
      computeAccessValues(x, y, query, grids, travelTimes, access)
    }
  }
 ...
 * @param {Object} grid
 * @param {Uint8Array} travelTimes
 * @param {Int32Array} accessPerMinute
 */

export function computeAccessValues (x, y, query, grids, travelTimes, access) {
  // get value of this pixel from grid
  for (let [name, grid] of grids) {
    let accessThisGrid = access.get(name)
    ...
    if (gridx >= 0 && gridx < grid.width && gridy >= 0 && gridy < grid.height) {
      ...
      for (let minute = 0; minute < travelTimes.length; minute++) {
        ...
        if (travelTime !== 255) {
          ...
          for (let cutoff = travelTime; cutoff < 120; cutoff++) {
            accessThisGrid[cutoff * travelTimes.length + minute] += val
          }
        }
      }
    }
