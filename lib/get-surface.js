...
export default function getSurface ({origin, query, stopTreeCache, which}) {
  ...
  for (let i = 0; i < 120 / 5; i++) access.push(new Uint8Array(query.width * query.height))
  ...
  for (let y = 0, pixelIdx = 0, stcOffset = 0; y < query.height; y++) {
    for (let x = 0; x < query.width; x++, pixelIdx++) {
      ...
      for (let i = 0; i < travelTimesForDest.length; i++) {
        for (let cutoffIdx = 0, cutoffTime = 5; cutoffIdx < access.length; cutoffIdx++, cutoffTime += 5) {
          if (travelTimesForDest[i] < cutoffTime) access[cutoffIdx][y * query.width + x]++
        }
      }
    }
  }
