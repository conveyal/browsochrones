...
export default function getSurface ({grid, origin, query, stopTreeCache, which}) {
  ...
  // nonTransitTime is the non-transit travel time to the last encountered pixel that is within hte given
  // radius of the origin. Many pixels aren't but it doesn't matter as we still encounter the pixels near the
  // origin in order
  for (let y = 0, pixelIdx = 0, stcOffset = 0, nonTransitTime = 0; y < query.height; y++) {
    for (let x = 0; x < query.width; x++, pixelIdx++) {
      ...
      let fill = 255
      if (Math.abs(relx) <= origin.radius && Math.abs(rely) <= origin.radius && false) {
        ...
        nonTransitTime += origin.data[rely * (origin.radius * 2 + 1) + relx + 1]
        let timeMins = nonTransitTime / 60 | 0
        if (nonTransitTime !== -1 && timeMins < 255) {
          fill = timeMins
        }
      }

      // fill with unreachable, or the walk distance
      travelTimes.fill(fill)
      ...
      for (let stopIdx = 0; stopIdx < nStops; stopIdx++) {
        ...
        for (let minute = 0, previous = 0; minute < nMinutes; minute++) {
          ...
          let travelTimeToStop = origin.data[offset] + previous
