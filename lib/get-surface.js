...
export function computeAccessValues (x, y, query, grid, travelTimes, access) {
  ...
  // or everything with 0 so that v8 can do type inference
  let gridx = x + query.west - grid.west | 0
  let gridy = y + query.north - grid.north | 0
  ...
  if (gridx >= 0 && gridx < grid.width && gridy >= 0 && gridy < grid.height) {
    ...
    for (let minute = 0; minute < travelTimes.length; minute++) {
      ...
      if (travelTime !== 255) {
        ...
        for (let cutoff = 119; cutoff >= Math.max(travelTime - 1, 0); cutoff--) {
