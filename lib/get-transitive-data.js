...
export default function getTransitiveData ({origin, query, stopTreeCache, network, to}) {
  ...
  for (let pidx = 0; pidx < paths.length; pidx++) {
    ...
    if (path.length > 20) {
      console.log('excessive path length, more than 20 segments')
      continue
    }
...
export function getPaths ({origin, to, stopTreeCache, query}) {
  ...
  let nonTransitTime = getNonTransitTime(origin, to)
...
export function bundle (data) {
  ...
  while (clusters.length > 1) {
    ...
    console.log(`dissimilarity ${minDis}`)
  ...
  console.log(`replaced ${inputSize} journeys with ${clusters.length} clusters`)
}
