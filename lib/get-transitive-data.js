...
function segmentDissimilarity (s1: Segment, s2: Segment, {
  ...
  const from1 = findStop(pat1.stops[s1.from_stop_index].stop_id)
  const to1 = findStop(pat1.stops[s1.to_stop_index].stop_id)
  const from2 = findStop(pat2.stops[s2.from_stop_index].stop_id)
  const to2 = findStop(pat2.stops[s2.to_stop_index].stop_id)
