...
export default function getTransitiveData ({ debug, origin, query, stopTreeCache, network, from, to }) {
  // create the journeys
  const output = {
    places: [],
    journeys: []
  ...
  let fromCoord = {}
  if (from != null) {
    const {x, y, ...rest} = from // eslint-disable-line
    fromCoord = ll(rest)
  }
  debug('fromCoord', fromCoord)
  output.places.push({
    place_id: 'from',
  ...
  const { x, y, ...rest } = to // eslint-disable-line
  ...
  let { paths } = getPaths({debug, origin, query, stopTreeCache, to})
  ...
  paths = paths.filter((p, idx, arr) =>
    idx === 0 || p[0] !== arr[idx - 1][0] || p[1] !== arr[idx - 1][1]).map(p =>
      getPath({debug, pathDescriptor: p, origin}))

  paths = filterPaths({debug, paths, network})
...
export function getPath ({debug, pathDescriptor, origin}) {
...
export function filterPaths ({debug, paths, network}) {
  ...
  paths = paths.filter((path, i, a) => {
    for (const otherPath of paths) {
      ...
      if (otherPathIsSubset) {
        debug(`path ${otherPath.map(seg => r(seg[1]))} is subset of path ${routes}`)
        return false
      }
    }
  ...
  debug(`filtering reduced ${inPathCount} paths to ${pathCountAfterMultiTransfer} after multiple-stop elimination, to ${paths.length} after stemming`)
 ...
 */
export function bundle ({data, debug}) {
