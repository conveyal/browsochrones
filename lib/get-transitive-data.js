...
// arbitrary value that determines how aggressively to cluster results (higher is more aggressive)
const MAXIMUM_DISSIMILARITY = 1
...
export function getPath ({pathDescriptor, origin}) {
  ...
  if (nSegments > MAX_PATH_SEGMENTS) {
    ...
    console.error(`Too many path segments (${nSegments} > ${MAX_PATH_SEGMENTS}) in path ${pathIdx} to stop ${stop} from origin ${origin.x}, ${origin.y}, returning no path. This implies a bug, please raise cain about it`)
    return null
  }
...
export function filterPaths (paths) {
  // for now this is pretty simple, sort paths by the sequence of patterns they use, and choose an example, eliminating the multiple-board/transfer-stops problem
  paths.sort((p1, p2) => {
  ...
  return paths.filter((p, i, a) => {
    if (i === 0) return true
    let prev = a[i - 1]
    if (p.length !== prev.length) return true

    for (let s = 0; s < p.length; s++) {
      if (p[s][1] !== prev[s][1]) return true
    }
...
export function bundle (data) {
  ...
  while (clusters.length > 1) {
    ...
    clusters = clusters.splice(minJ, 1) // remove clusters[j]
  }
}
...
function clusterDissimilarity (c1, c2, data) {
  ...
  for (let j1 of c1) {
    for (let j2 of c2) {
      ...
      if (j1.length !== j2.length) return Infinity

      // otherwise compute maximum dissimilarity of stops at either start or end
      for (let segment = 0; segment < j1.length; segment++) {
        // stop at start of segment
        ret = Math.max(stopDistance(j1[segment][0], j2[segment][0], data))
        // end of segment
        ret = Math.max(stopDistance(j1[segment][2], j2[segment][2], data))
...
function stopDistance (stop1, stop2, data) {
  ...
  let s1 = data.stops.filter(s => s.stop_id === stop1 + '')[0]
  let s2 = data.stops.filter(s => s.stop_id === stop2 + '')[0]

  let cosLat = Math.cos(s1.stop_lat * Math.PI / 180)
  return Math.pow(s1.stop_lat - s2.stop_lat) + Math.pow(s1.stop_lon * cosLat - s2.stop_lon * cosLat)
}
